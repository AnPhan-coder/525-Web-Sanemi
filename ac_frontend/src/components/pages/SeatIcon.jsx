...
const SeatIcon = ({ 
  ...
  if (isHidden) return <div className={`opacity-0 ${className || "w-8 h-8 md:w-10 md:h-10"}`}></div>;

  let fillColor = "#525252";
  let strokeColor = "none";
  let cursorStyle = onClick 
    ? (isBooked ? "cursor-not-allowed opacity-40" : "cursor-pointer hover:brightness-110 hover:-translate-y-1") 
    : "";
  let shadowClass = "";
  ...
  const handleClick = (e) => {
    if (onClick && !isBooked) onClick(e);
  };

  const defaultCoupleClass = className ? className : "w-20 h-9 sm:w-[5.5rem] sm:h-10";
  const defaultNormalClass = className ? className : "w-9 h-9 sm:w-10 sm:h-10";

  if (type === "COUPLE") {
    return (
      <div
        onClick={handleClick}
        className={`relative flex justify-center items-end group transition-all duration-300 ${cursorStyle} ${shadowClass} ${defaultCoupleClass}`}
        title={title}
      >
  ...
  return (
    <div
      onClick={handleClick}
      className={`relative flex justify-center items-end group transition-all duration-300 ${cursorStyle} ${shadowClass} ${defaultNormalClass}`}
      title={title}
    >
